...
function buildInternetDocumentDescription(order = {}) {
  const description = cleanNpText(env('NP_DESCRIPTION'));
  const product = cleanNpText(order.product || process.env.PRODUCT_NAME || 'Order');
  const size = cleanNpText(order.size || '');
...
function buildInternetDocumentAdditionalInformation(description) {
  return cleanNpText(env('NP_ADDITIONAL_INFORMATION') || description);
}

function buildNovaPaymentAmount(order = {}) {
  const basePrice = Math.max(1, moneyRounded(order.price || process.env.PRODUCT_PRICE || 0));
  const extra = Math.max(0, moneyRounded(env('NP_PAYMENT_CONTROL_EXTRA_UAH', '140')));
  return {
    basePrice,
    extra,
    amount: basePrice + extra,
  };
}
...
async function createInternetDocument(order) {
  ...
  const payment = buildNovaPaymentAmount(order);
  ...
  if (envFlag('NP_PAYMENT_CONTROL_ENABLED', 'true') && payment.amount > 0) {
    ...
    properties.BackwardDeliveryData = [{
      ...
      CargoType: 'Money',
      RedeliveryString: String(payment.amount),
    }];
  }
...
async function trackDocuments(documents) {
  ...
  const tracks = (result.data || []).map(item => ({
    ...
    warehouseAddress: item.WarehouseRecipientAddress || item.RecipientAddress || '',
    warehouseNumber: item.WarehouseRecipientNumber || '',
    warehouseType: item.CategoryOfWarehouse || item.RecipientWarehouseTypeRef || '',
    senderCity: item.CitySender || item.CitySenderDescription || '',
    senderWarehouse: item.WarehouseSender || item.WarehouseSenderDescription || item.WarehouseSenderAddress || '',
    senderWarehouseAddress: item.WarehouseSenderAddress || item.SenderAddress || '',
    sentAt: item.DateCreated || '',
    receivedAt: item.ActualDeliveryDate || item.RecipientDateTime || '',
    dateReturnCargo: item.DateReturnCargo || '',
    dateMoving: item.DateMoving || '',
    firstDayStorage: item.DateFirstDayStorage || '',
    daysStorageCargo: item.DaysStorageCargo || '',
    storageAmount: moneyNumber(item.StorageAmount || 0),
    storagePrice: moneyNumber(item.StoragePrice || 0),
    possibilityCreateReturn: item.PossibilityCreateReturn === true || item.PossibilityCreateReturn === 'true',
    possibilityCreateRefusal: item.PossibilityCreateRefusal === true || item.PossibilityCreateRefusal === 'true',
    possibilityCreateRedirecting: item.PossibilityCreateRedirecting === true || item.PossibilityCreateRedirecting === 'true',
    possibilityLightReturn: item.PossibilityLightReturn === true || item.PossibilityLightReturn === 'true',
    cargoReturnRefusal: item.CargoReturnRefusal === true || item.CargoReturnRefusal === 'true',
    createdOnTheBasis: item.CreatedOnTheBasis || '',
    ownerDocumentNumber: item.OwnerDocumentNumber || '',
    lastCreatedOnTheBasisNumber: item.LastCreatedOnTheBasisNumber || '',
    lastCreatedOnTheBasisDocumentType: item.LastCreatedOnTheBasisDocumentType || '',
    lastCreatedOnTheBasisDateTime: item.LastCreatedOnTheBasisDateTime || '',
...
function configStatus() {
  ...
  return {
    ...
    paymentControlExtraUah: moneyRounded(env('NP_PAYMENT_CONTROL_EXTRA_UAH', '140')),
