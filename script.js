...
const aiOrderForm = document.getElementById('aiOrderForm');
const aiOrderInput = document.getElementById('aiOrderInput');
const aiOrderMessages = document.getElementById('aiOrderMessages');
const aiOrderStatus = document.getElementById('aiOrderStatus');
const aiOrderReset = document.getElementById('aiOrderReset');
const aiOrderMessage = document.getElementById('aiOrderMessage');

let aiOrderSessionId = sessionStorage.getItem('vm_ai_order_session') || '';
let aiOrderSending = false;
let aiOrderCompleted = false;
let aiOrderStarted = false;

function addAiOrderMsg(text, isUser = false) {
  if (!aiOrderMessages) return;
  const div = document.createElement('div');
  div.className = `ai-order-msg ${isUser ? 'ai-order-msg--user' : 'ai-order-msg--bot'}`;
  div.textContent = text;
  aiOrderMessages.appendChild(div);
  aiOrderMessages.scrollTop = aiOrderMessages.scrollHeight;
}

function showAiTyping() {
  if (!aiOrderMessages || document.getElementById('aiOrderTyping')) return;
  const div = document.createElement('div');
  div.className = 'ai-order-msg ai-order-msg--bot ai-order-typing';
  div.id = 'aiOrderTyping';
...
function hideAiTyping() {
  const el = document.getElementById('aiOrderTyping');
  if (el) el.remove();
}

function setAiOrderBusy(busy) {
  aiOrderSending = busy;
  const btn = aiOrderForm?.querySelector('.ai-order-send');
  if (btn) {
    btn.disabled = busy || aiOrderCompleted;
    btn.textContent = busy ? 'Працюю...' : 'Надіслати';
  }
  if (aiOrderInput) aiOrderInput.disabled = busy || aiOrderCompleted;
  if (aiOrderStatus) aiOrderStatus.textContent = busy ? 'Gemini перевіряє дані...' : (aiOrderCompleted ? 'замовлення оформлено' : 'оформлю замовлення без дзвінка оператора');
}

function resetAiOrderChat() {
  aiOrderSessionId = '';
  aiOrderCompleted = false;
  aiOrderStarted = false;
  sessionStorage.removeItem('vm_ai_order_session');
  if (aiOrderInput) { aiOrderInput.value = ''; aiOrderInput.disabled = false; }
  if (aiOrderMessages) {
    aiOrderMessages.innerHTML = '';
    addAiOrderMsg("Вітаю! Я допоможу оформити замовлення на Violet Motion. Напишіть ім'я, телефон, розмір, місто та відділення Нової пошти. Якщо зручніше, можна надсилати по одному пункту.");
  }
  if (aiOrderMessage) showMsg(aiOrderMessage, '', '');
  setAiOrderBusy(false);
  setTimeout(() => aiOrderInput?.focus(), 80);
}

if (aiOrderInput) {
  aiOrderInput.addEventListener('focus', () => {
    if (!aiOrderStarted) {
      aiOrderStarted = true;
...
aiOrderForm?.addEventListener('submit', async e => {
  e.preventDefault();
  if (aiOrderSending || aiOrderCompleted) return;

  const text = aiOrderInput.value.trim();
  if (!text) return showMsg(aiOrderMessage, 'Напишіть відповідь для оформлення замовлення.', 'error');

  showMsg(aiOrderMessage, '', '');
  addAiOrderMsg(text, true);
  aiOrderInput.value = '';
  setAiOrderBusy(true);
  showAiTyping();
  Analytics.track('form_submit', { mode: 'gemini' });

  const result = await postJSON('/api/order/ai/message', {
    sessionId: aiOrderSessionId,
    message: text,
  }, 25000);
  ...
  if (!result || !result.success) {
    ...
    const msg = result?.timeout
      ? 'Сервер не відповідає. Спробуйте ще раз або напишіть у підтримку.'
      : 'Не вдалося обробити відповідь. Спробуйте ще раз.';
    return showMsg(aiOrderMessage, msg, 'error');
  }

  if (result.sessionId) {
    aiOrderSessionId = result.sessionId;
    sessionStorage.setItem('vm_ai_order_session', aiOrderSessionId);
  }

  if (result.reply) addAiOrderMsg(result.reply, false);

  if (result.completed && result.order) {
    aiOrderCompleted = true;
    sessionStorage.removeItem('vm_ai_order_session');
    await ttIdentifyPhone(result.order.phone || '');
    ...
    Analytics.track('order_success', { size: result.order.size, mode: 'gemini' });
    showMsg(aiOrderMessage, 'Замовлення передано менеджеру. Дякуємо!', 'success');
    showSuccessOverlay(result.order.name || '', result.order.size || '');
  }

  setAiOrderBusy(false);
  if (!aiOrderCompleted) aiOrderInput.focus();
});

aiOrderReset?.addEventListener('click', resetAiOrderChat);
